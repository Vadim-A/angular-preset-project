...
`@angular/localize` - для интернационализации.

## Команды проекта

- `start`: запуск проекта.
- `start:en-US`: запуск проекта с английской локализацией
- `build`: сборка проекта.
- `build:prod`: сборка проекта в режиме prod (с оптимизацией и без source map).
- `build:prod-localize`: сборка проекта под все локали.
- `test`: прогон тестов.
...
- `extract-i18n`: извлечение помеченых фраз для перевода.
...
При таком режиме разработки, прсле каждого изменения нужно самому обновлять браузер с очищением кэша.
